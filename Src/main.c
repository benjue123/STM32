...
uint8_t DHT22_Start (void)
{
  ...
  GPIO_InitStructPrivate.Pull = GPIO_NOPULL; // 因為已設定 push pull, 所以這邊不用再設上下拉，影響不大
...
uint8_t DHT22_Read (void)
{
  ...
  for (a=0;a<8;a++)
  {
    ...
    microDelay (40);   // wait for 40 us to recognize '0' or '1', since  bit means '0', high time last 22-30 us. But bit means '1', high time last 68-75 us
    if (!(HAL_GPIO_ReadPin (DHT22_PORT, DHT22_PIN)))   // if the pin is low, it means this bit is '0'
      b&= ~(1<<(7-a));
    else
      b|= (1<<(7-a));
    pMillis = HAL_GetTick();
    cMillis = HAL_GetTick();
    while ((HAL_GPIO_ReadPin (DHT22_PORT, DHT22_PIN)) && pMillis + 2 > cMillis)
    {  // wait for the pin to go low, it means this bit end
      cMillis = HAL_GetTick();
    }
  }
  return b;
...
void read_sensor_and_send(void)
{
	if(DHT22_Start())
		  	      {
